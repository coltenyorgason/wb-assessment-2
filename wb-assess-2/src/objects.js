...
function getNameUsingDestructuring(person) {
  return person.map(({ firstName, lastName }) => `${firstName} ${lastName}`);
}
...
function getPeopleByLocation(people, location) {
  return people.filter((i) => i.location == location);
}
...
function translateToPirateTalk(phrase) {
  const eachWord = phrase.split(" ");
  let newPhrase = "";
  eachWord.forEach((e) => {
    EN_PIRATE_LOOKUP[e]
      ? (newPhrase += `${EN_PIRATE_LOOKUP[e]} `)
      : (newPhrase -= `${e} `);
  });
  return newPhrase;
}
...
function wordCount(str) {
  ...
  let object = {}
  word.forEach((e) => {
    object[e] ? object[e] += 1 : object[e] = 1
  })
  return object
}
