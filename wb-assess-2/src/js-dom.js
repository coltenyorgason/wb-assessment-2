...
myButton.addEventListener("click", () => {
  myButton.innerHTML === "Log-in"
    ? (myButton.innerHTML = "Log-out")
    : (myButton.innerHTML = "Log-in");
});
...
document.querySelector("#send-alert").addEventListener("submit", (e) => {
  e.preventDefault();
...
document.querySelector("#blue").addEventListener("click", (e) => {
 const colour = document.querySelectorAll(".changes-colors")
 colour.forEach(i => i.style.color = "blue");
});
document.querySelector("#red").addEventListener("click", (e) => {
    const colour = document.querySelectorAll(".changes-colors")
    colour.forEach(i => i.style.color = "red");
});
...
document.querySelector("#factorial-calculator").addEventListener("submit", e => {
    e.preventDefault()
    const factorialInput = document.querySelector("#factorial-input").value
    let fact = 1
    for(let i = 2; i <= factorialInput; i++) {
        fact = fact * i
    }
document.querySelector("#result").innerHTML = fact
})
...
document.querySelector("#recommend-word").addEventListener("submit", e => {
    e.preventDefault()
    const validateForm = document.querySelector("#word").value
    let form = 0
    let feedback = document.querySelector(".form-feedback")
    if(validateForm.length < 3 ) { 
        feedback.innerHTML = "The word must be at least 4 characters long."
